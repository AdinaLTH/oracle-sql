...
INSERT INTO product_tb1 (pcode, pname, price, pdscb, rate, pcomp)
VALUES((SELECT NVL(MAX(pcode), 0)+1 FROM product_tb1)
     ,'애플펜슬PRO'
     ,200000
     ,'전작보다 향상된 필기감'
     ,5
...
SELECT 'S'||
        (SELECT LPAD(MAX(SUBSTR(pcode, 1, 1)), 3, '0') FROM product_tb1)
FROM dual;
...
USING (SELECT 1 pcode
             ,'애플펜슬PRO' pname
             ,198000 price
             ,'전작보다 향상된 부드러운 필기감' pdscb
        FROM dual) tbl2
ON (tbl1.pcode = tbl2.pcode)
WHEN MATCHED THEN
    UPDATE SET
        tbl1.pname = tbl2.pname
       ,tbl1.price = tbl2.price
       ,tbl1.pdscb = tbl2.pdscb
WHEN NOT MATCHED THEN
    INSERT (pcode, pname, price, pdscb)
    VALUES (tbl2.pcode, tbl2.pname, tbl2.price, tbl2.pdscb);